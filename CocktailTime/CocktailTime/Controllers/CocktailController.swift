...
class CocktailController: UIViewController {
    ...
    func getCocktailIngredients2 (_ cocktailId: Ingredients, completion: @escaping (([Ingredients?])) -> Void) {
        let id = cocktailId.id
        
        guard var url = CocktailController.baseUrl else {
            completion([nil])
            return
        }
        ...
        url.appendPathComponent("json")
        url.appendPathComponent("v1")
        url.appendPathComponent("\(CocktailController.apiSecret)")
        ...
        URLSession.shared.dataTask(with: request) { (data, response, error) in
            ...
            do {
                let ingredientService = try jsonDecoder.decode(IngredientService.self, from: data)
                let ingredients = ingredientService.ingredientResults
